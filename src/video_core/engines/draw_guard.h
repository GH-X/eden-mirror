...
/*
    ...
    //in DrawManager::ProcessDraw (after UpdateTopology)
    if (ShouldDiscardCorruptedDraw(draw_state, nullptr, draw_indexed, instance_count)) {
        return;
    }

    //in DrawManager::ProcessDrawIndirect (after UpdateTopology)
...
[[nodiscard]] inline bool ShouldDiscardCorruptedDraw(const DrawManager::State& draw_state,
                                                     const DrawManager::IndirectParams* indirect_state,
    ...
    const bool validate_count = (indirect_state == nullptr) || draw_indexed;
